...
public class FreeDrawShape extends DrawableObject {
    ...
    public DrawableObject checkShape() {
        DrawableObject obj = isLine();
        if(obj != null) {
            return obj;
        }
        else {
            if(MathUtils.distancePoints(points.get(0), points.get(points.size() - 1)) > ERROR_MARGIN * 4) {
                return null;
            }
            obj = isCircle();
            if(obj != null) {
    ...
    private Line isLine() {
        ...
        return new Line(this.getColour(), this.getColour(), start.getX(), start.getY(), start, finish, (int) this.getRadius());
    }
    ...
    public void move(int x, int y) {
        ...
        for(int i = 0; i < points.size(); ++i) {
            points.set(i, new Point(points.get(i).getX() + x, points.get(i).getY() + y));
        }
    }
