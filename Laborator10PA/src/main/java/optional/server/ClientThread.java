...
import optional.utils.DataBaseConnection;
...
public class ClientThread extends Thread
{
    ...
    public ClientThread(Socket socket, DataBaseConnection dbc)
    {
        this.socket = socket;
        this.dbc = dbc;
    }
    ...
    private String interpretCommand(String string) {
        ...
        {
            case "register":
                com = new CommandRegister(command[1], this.dbc.getConnection());
                result = com.run();
            ...
            case "login":
                com = new CommandLogin(command[1], this.dbc.getConnection());
                result = com.run();
                if(result.equals("succ")) {
            ...
            case "friend":
                ...
                {
                    com = new CommandFriend(clientName, s, this.dbc.getConnection());
                    result = com.run();
            ...
            case "send":
                ...
                com = new CommandSend(command[command.length - 1], command[1], text.toString().trim(), this.dbc.getConnection());
                result = com.run();
                if(result.equals("succ"))
                {
                    return "Message sent";
            ...
            case "read":
                com = new CommandRead(command[1], this.dbc.getConnection());
                result = com.run();
