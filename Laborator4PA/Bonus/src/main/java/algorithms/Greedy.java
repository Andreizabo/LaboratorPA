...
public class Greedy implements Algorithm {
    ...
    public Solution solve(Problem problem) {
        NormalProblem normalProblem = (NormalProblem)problem;
        Solution solution = new Solution();

        List<Student> sortedStudents = new ArrayList<>(normalProblem.getStudents());
        ...
        normalProblem.getSchools().forEach((s) -> schoolCapacities.put(s.getName(), s.getCapacity()));

        for(Student student : sortedStudents) {
            for(School school : normalProblem.getStudentPreferences().get(student)) {
